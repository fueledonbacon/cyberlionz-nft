...
contract CyberlionStaking is Ownable {
	...
	function _claimReward(address _userAddress, uint256 _collectionID)
		internal
	{
		UserInfo storage user = userInfo[_userAddress];
		CollectionInfo storage collection = collectionInfo[_collectionID];
		uint256 payableAmount = (block.timestamp -
			user.timeStaked[collection.collectionAddress])
			.div(SECONDS_PER_DAY)
            .mul(10**18)
			.mul(collection.rewardPerDay);
		mintMinerReward(payableAmount, _userAddress);
		// rewardsTokenAddress.transferFrom(
		// 	heatClAddress,
		// 	msg.sender,
		// 	payableAmount
		// );
	...
	function mintMinerReward(uint256 payableAmount, address _user) public {
		Mintable(rewardsTokenAddress).mint(_user, payableAmount);
	}

	function setCollection(address _collectionAddress, uint256 _rewardPerDay)
		public
	{
		collectionInfo.push(
			CollectionInfo({
	...
	function getUserInformation(address _userAddress, address _collectionAddr)
		external
		view
		returns (
			uint256[] memory,
	...
	{
		return (
			userInfo[_userAddress].stakedTokens[_collectionAddr],
			userInfo[_userAddress].timeStaked[_collectionAddr],
			userInfo[_userAddress].amountStaked
		);
	}

	function getTotalStakedItemsCount(uint256 _collectionID)
		external
		view
		returns (uint256)
	{
